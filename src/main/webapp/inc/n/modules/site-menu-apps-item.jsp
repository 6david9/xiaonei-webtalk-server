...
<c:forEach var="item" items="${requestScope.userMenu.recentMenu}" begin="${begin}" end="${end}">
	...
	<c:when test="${item.appId==45}"><%-- 状态 --%>
		<div class="apps-item with-menu">
			<div class="item-title"><span class="item-main"><a href="http://status.renren.com/?fromType=nomore&amp;from=homeleft" ui-async="async"><img class="icon" src="${item.appIcon}" /> ${item.appName}</a></span></div>
			<ul>
				<li><a href="http://status.renren.com/status?id=${visiter.id}&amp;from=homeleft" ui-async="async">我的状态</a></li>
				<li><a href="http://status.renren.com/status/hot/list" ui-async="async">人人聚焦</a></li>
			</ul>
		</div>
	</c:when>
